...
```{r message=False, warning=False, results='asis'}
crime_london$boro_name = substr(crime_london$lsoa_name,1,nchar(crime_london$lsoa_name)-4)
...
What are the most common outcome of people being S&S?
...
What are the most common ethnicity of successful S&S?
...
df <- data.frame(merged_boro) %>% 
  dplyr::select(NAME, GSS_CODE, ss_rate, arrest_rate, crime_rate)
...
1\. Elbow Method 2. Average Silhouette Method
...
```{r}
ggplot(merged_boro, aes(x='', y=ss_rate, color=cluster)) +
  geom_boxplot()
```
