...
```{r}
plotData <- merged_boro %>%
  dplyr::select(boro_name,
                hit_rate,
                crime_rate) %>%
  pivot_longer(cols = (hit_rate |crime_rate),
               names_to = "names",
               values_to = "rates")

ggplot(plotData, aes(x=boro_name, y=rates, group=names)) +
...
regression_data <- regression_data %>% st_as_sf()
tm_shape(regression_data) +
  tm_polygons(col = "coef_Whiteinner", 
              palette = "-RdBu", 
              alpha = 0.5)
```
...
```{r}
#calculate kernel bandwidth
GWRbandwidth <- gwr.sel(asian_arrest_rate ~ asian_search_rate + inner_outer,
                  data = regression_data_SP,
                        coords=coordsMSP,
                        adapt=T)
```
...
```{r}
#run the gwr model
model_gwr3 = gwr(asian_arrest_rate ~ asian_search_rate + inner_outer,
                  data = regression_data_SP, 
                coords=coordsMSP, 
                adapt=GWRbandwidth, 
                hatmatrix=TRUE, 
...
regression_data <- regression_data %>%
  mutate(coef_Asiansearch = results$asian_search_rate,
         coef_Asianinner = results$inner_outerInner.London)
```
...
tm_shape(regression_data) +
  tm_polygons(col = "coef_Asianinner", 
              palette = "-RdBu", 
              alpha = 0.5)
...
sigTest_Asianinner = abs(model_gwr3$SDF$"inner_outerInner.London") - (2* model_gwr3$SDF$"inner_outerInner.London_se")

#store significance results
regression_data <- regression_data %>%
  mutate(se_asiansearch = sigTest_Asiansearch,
         se_asianinner = sigTest_Asianinner)
...
asianGWR$coef_Asianinner <- replace(asianGWR$coef_Asianinner, 
                    asianGWR$se_asianinner < 0,
                    NA) 
...
finalGWR$coef_CrimeLog <- replace(finalGWR$coef_CrimeLog, 
                    finalGWR$se_crimeLog < 0,
                    NA) 

finalGWR$coef_meadianIncome <- replace(finalGWR$coef_meadianIncome, 
                    finalGWR$se_medianIncome < 0,
                    NA) 

finalGWR$coef_oldDeprivation <- replace(finalGWR$coef_oldDeprivation, 
                    finalGWR$se_deprivation < 0,
                    NA) 
...
finalGWR$coef_inner_outerInner <- replace(finalGWR$coef_inner_outerInner, 
                    finalGWR$se_innerOuter < 0,
                    NA) 
