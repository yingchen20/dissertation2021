...
ggplot(all_data, aes(x=total_search_rate)) + 
...
ggplot(all_data, aes(x=total_hit_rate)) + 
  geom_histogram(aes(y = ..density..),
                 binwidth = 5) + 
  geom_density(colour="red", 
               size=1, 
...
3.  total violent crime rate

```{r}
summary(all_data$total_vio_rate)

ggplot(all_data, aes(x=total_vio_rate)) + 
  geom_histogram(aes(y = ..density..),
                 binwidth = 1) + 
  geom_density(colour="red", 
               size=1, 
...
4.  total search counts

```{r}
summary(all_data$total_ss60)
...
5.  total arrest counts

```{r}
summary(all_data$total_arrest)
...
6.  total violent crime counts

```{r}
summary(all_data$total_violent_crime)

ggplot(all_data, aes(x=total_violent_crime)) + 
  geom_histogram(aes(y = ..density..),
                 binwidth = 50) + 
  geom_density(colour="red", 
               size=1, 
...
#### arrest count & search count

```{r message=FALSE, warning=FALSE}
library(broom)
all_data[is.na(all_data)] <- 0

q <- qplot(x = total_ss60, 
           y = total_arrest,
           data = all_data)
...
model1 <- all_data %>%
  lm(total_arrest ~ total_ss60, data=.)
...
#### hit rate & search rate 

Q: scatter plot shows non-linear patterns, should I take log-transformation to my variables or fit to a non-linear model?

```{r message=FALSE, warning=FALSE}
q <- qplot(x = total_search_rate, 
           y = total_hit_rate,
           data = all_data)
...
model1 <- all_data %>%
  lm(total_hit_rate ~ total_search_rate, data=.)
...
```{r}
ggplot(all_data, aes(x=log(total_search_rate)) )+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 0.5) + 
  geom_density(colour="red", 
               size=1, 
...
```{r}
ggplot(all_data, aes(x=(total_search_rate)^-1))+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 5) + 
  geom_density(colour="red", 
               size=1, 
