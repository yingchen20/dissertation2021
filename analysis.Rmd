...
Rates are very inconsistent across boroughs

```{r}
summary(merged_boro$crime_rate)
...
msoaDataMerged <-
  ...
  rename(all_population = "age_structure_2011_census_all_ages",
         ...
         no_qualification = "qualifications_2011_census_no_qualifications")
...
p2 <- tm_shape(all_data) + 
  tm_fill("black_pop", 
          style="jenks",
          palette="viridis",
          title="Black Population Percent (Jenks)")+
  tm_legend(show=T)+
  tm_layout(frame=FALSE,
            legend.title.size = 0.5,
            legend.text.size = 0.4,
...
p4 <- tm_shape(all_data) + 
  tm_fill("white_pop", 
          style="jenks",
          palette="viridis",
          title="White Population Percent (Jenks)")+
  tm_legend(show=T)+
  tm_layout(frame=FALSE,
            legend.title.size = 0.5,
            legend.text.size = 0.4,
...
p5 <- tm_shape(all_data) + 
  tm_fill("asian_search_rate", 
          style="jenks",
          palette="viridis",
...
p6 <- tm_shape(all_data) + 
  tm_fill("asian_pop", 
          style="jenks",
          palette="viridis",
          title="Asian Population Percent (Jenks)")+
  tm_legend(show=T)+
  tm_layout(frame=FALSE,
            legend.title.size = 0.5,
            legend.text.size = 0.4,
...
p <- tmap_arrange(p1, p2, p3, p4, p5, p6)
#tmap_save(p, "output/hit_rate_map2.png",dpi=500)
p
```
...
test_black$boro_name = substr(test_black$msoa_name,1,nchar(test_black$msoa_name)-4)
summary(all_data$black_search_rate)

test_black <- test_black %>% 
  mutate(black_ss_compare = case_when(
      (black_search_rate < 23.60) ~ "Below London Average",
      TRUE ~ "Above London Average"))

test_black %>% 
  group_by(black_ss_compare)%>%
  summarise(., count=n()) %>% 
  kable()
...
test_white$boro_name = substr(test_white$msoa_name,1,nchar(test_white$msoa_name)-4)
summary(all_data$white_search_rate)

test_white <- test_white %>% 
  mutate(white_ss_compare = case_when(
      (white_search_rate < 4.901) ~ "Below London Average",
      TRUE ~ "Above London Average"))

test_white %>% 
  group_by(white_ss_compare)%>%
  summarise(., count=n()) %>% 
  kable()
```
...
test_0 %>% 
  filter(asian_ss_compare=="Above London Average") %>% 
  kable()
```
...
```{r}
all_rates <- all_data %>% 
  dplyr::select(ends_with("rate") | msoa_code | black_pop | white_pop | asian_pop)
```
...
# population
all_rates$s_black_pop <- scale(all_rates$black_pop) %>% as.vector()
all_rates$s_white_pop <- scale(all_rates$white_pop) %>% as.vector()
all_rates$s_asian_pop <- scale(all_rates$asian_pop) %>% as.vector()
...
# population
all_rates$lag_s_black_pop <- lag.listw(Lward_lw, all_rates$s_black_pop)
all_rates$lag_s_white_pop <- lag.listw(Lward_lw, all_rates$s_white_pop)
all_rates$lag_s_asian_pop <- lag.listw(Lward_lw, all_rates$s_asian_pop)
...
```{r}
all_rates <- all_rates %>% 
  mutate(quad_sig_black_pop = ifelse(all_rates$s_black_pop > 0 & 
                              all_rates$lag_s_black_pop > 0 & 
                              moranI_black_pop[,5] <= 0.05, 
                     "high-high",
                     ifelse(all_rates$s_black_pop <= 0 & 
                              all_rates$lag_s_black_pop <= 0 & 
                              moranI_black_pop[,5] <= 0.05, 
                     "low-low", 
                     ifelse(all_rates$s_black_pop > 0 & 
                     ...
                     "low-high", 
                     "non-significant")))),
      quad_sig_white_pop = ifelse(all_rates$s_white_pop > 0 & 
                              all_rates$lag_s_white_pop > 0 & 
                              moranI_white_pop[,5] <= 0.05, 
                     "high-high",
                     ifelse(all_rates$s_white_pop <= 0 & 
                              all_rates$lag_s_white_pop <= 0 & 
                              moranI_white_pop[,5] <= 0.05, 
                     "low-low", 
                     ifelse(all_rates$s_white_pop > 0 & 
                     ...
                     "low-high", 
                     "non-significant")))),
      quad_sig_asian_pop = ifelse(all_rates$s_asian_pop > 0 & 
                              all_rates$lag_s_asian_pop > 0 & 
                              moranI_asian_pop[,5] <= 0.05, 
                     "high-high",
                     ifelse(all_rates$s_asian_pop <= 0 & 
                              all_rates$lag_s_asian_pop <= 0 & 
                              moranI_asian_pop[,5] <= 0.05, 
                     "low-low", 
                     ifelse(all_rates$s_asian_pop > 0 & 
...
t1 <- tm_shape(all_rates) + 
  tm_fill("quad_sig_black_search",
        palette= "RdGy",
        midpoint=NA,
        title="LISA, Black Search Hotpots") +
...
t3 <- tm_shape(all_rates) + 
  tm_fill("quad_sig_white_search",
        palette= "RdGy",
        midpoint=NA,
        title="LISA, White Search Hotpots") +
    tm_borders(NA, alpha = 0.3) +
  tm_legend(show=T)+
  tm_layout(frame=FALSE,            
            legend.outside.size = 1,
...
t5 <- tm_shape(all_rates) + 
  tm_fill("quad_sig_asian_search",
        palette= "RdGy",
        midpoint=NA,
        title="LISA, Asian Search Hotpots") +
  tm_borders(NA, alpha = 0.3) +
  tm_legend(show=T)+
  tm_layout(frame=FALSE,            
            legend.outside.size = 1,
...
t2 <- tm_shape(all_rates) + 
  tm_fill("quad_sig_black_pop",
        palette= "RdGy",
        midpoint=NA,
        title="LISA, Black Pop. Hotpots") +
    tm_borders(NA, alpha = 0.3) +
  tm_legend(show=T)+
  tm_layout(frame=FALSE,
            legend.outside.size = 1,
...
t4 <- tm_shape(all_rates) + 
  tm_fill("quad_sig_white_pop",
        palette= "RdGy",
        midpoint=NA,
        title="LISA, White Pop. Hotpots") +
    tm_borders(NA, alpha = 0.3) +
  tm_legend(show=T)+
  tm_layout(frame=FALSE,            
            legend.outside.size = 1,
...
t6 <- tm_shape(all_rates) + 
  tm_fill("quad_sig_asian_pop",
        palette= "RdGy",
        midpoint=NA,
        title="LISA, Asian Pop. Hotpots") +
    tm_borders(NA, alpha = 0.3) +
  tm_legend(show=T)+
  tm_layout(frame=FALSE,            
            legend.outside.size = 1,
...
t7 <- tm_shape(all_rates) + 
  tm_fill("quad_sig_total_crime",
        palette= "RdGy",
        midpoint=NA,
        title="LISA, Crime Hotpots") +
    tm_borders(NA, alpha = 0.3) +
  tm_legend(show=T)+
  tm_layout(frame=FALSE,            
            legend.outside.size = 1,
...
t=tmap_arrange(t1, t2, t3, t4, t5, t6, t7,
               nrow = 4, ncol = 2)
t
tmap_save(t, "output/lisa_map_withPOP.png",dpi=500)
```
...
Moderate spatial autocorrelation so GWR. 
...
model_b <- regression_data %>% 
  lm(black_arrest_rate ~ black_search_rate + inner_outer, data=.)
...
```{r}
#calculate kernel bandwidth
GWRbandwidth <- gwr.sel(black_arrest_rate ~ black_search_rate + inner_outer,
                  data = regression_data_SP,
                        coords=coordsMSP,
                        adapt=T)
```
...
```{r}
#run the gwr model
model_gwrBlack = gwr(black_arrest_rate ~ black_search_rate + inner_outer,
                  data = regression_data_SP, 
                coords=coordsMSP, 
                adapt=GWRbandwidth, 
                hatmatrix=TRUE, 
...
```{r}
results <- as.data.frame(model_gwrBlack$SDF)
names(results)
```

```{r}
regression_data <- regression_data %>%
  mutate(coef_Blacksearch = results$black_search_rate,
         coef_blackInner = results$inner_outerInner.London)
```


these results may not be statistically significant across the whole of London. Roughly speaking, if a coefficient estimate is more than 2 standard errors away from zero, then it is "statistically significant."

```{r}
#run the significance test
sigTest_Blacksearch = abs(model_gwrBlack$SDF$"black_search_rate") - (2* model_gwrBlack$SDF$"black_search_rate_se")

sigTest_BlackInner = abs(model_gwrBlack$SDF$"inner_outerInner.London") - (2* model_gwrBlack$SDF$"inner_outerInner.London_se")

#store significance results
regression_data <- regression_data %>%
  mutate(se_Blacksearch = sigTest_Blacksearch,
         se_BlackInner = sigTest_BlackInner)
```

prepare for the final plot

```{r}
blackGWR <- subset(regression_data) 
# replace non-sig to na
blackGWR$coef_Blacksearch <- replace(blackGWR$coef_Blacksearch, 
                    blackGWR$se_Blacksearch < 0,
                    NA) 

blackGWR$coef_blackInner <- replace(blackGWR$coef_blackInner, 
                    blackGWR$se_BlackInner < 0,
                    NA) 
...
```{r}
tm3 <- tm_shape(blackGWR) + 
  tm_fill("coef_Blacksearch", 
          palette= "viridis",
          style = "jenks",
          title = "GWR, black search rate coef.",
...
tm4 <- tm_shape(blackGWR) + 
  tm_fill("coef_blackInner", 
          palette= "viridis",
          style = "jenks",
          title = "GWR, black Inner London coef.",
          textNA = "Non-significant",
          title.fontface = "bold",
          alpha = 0.8)+  
  tm_borders(col = NA, alpha = 0.1) +
...
model_w <- regression_data %>% 
  lm(white_arrest_rate ~ white_search_rate + inner_outer, data=.)
...
regression_data <- model_w %>%
  augment(., regression_data) %>% 
  rename(white_resid2 = ".resid")

#plot residuals
regression_data%>%
  dplyr::select(white_resid2)%>%
  pull()%>%
  qplot()+ 
  geom_histogram()
...
```{r}
#calculate kernel bandwidth
GWRbandwidth <- gwr.sel(white_arrest_rate ~ white_search_rate +inner_outer,
                  data = regression_data_SP,
                        coords=coordsMSP,
                        adapt=T)
```
...
```{r}
#run the gwr model
model_gwr2 = gwr(white_arrest_rate ~ white_search_rate + inner_outer,
                  data = regression_data_SP, 
                coords=coordsMSP, 
                adapt=GWRbandwidth, 
                hatmatrix=TRUE, 
...
regression_data <- regression_data %>%
  mutate(coef_Whitesearch = results$white_search_rate,
         coef_whiteInner = results$inner_outerInner.London)
```
...
sigTest_WhiteInner = abs(model_gwr2$SDF$"inner_outerInner.London") - (2* model_gwr2$SDF$"inner_outerInner.London_se")

#store significance results
regression_data <- regression_data %>%
  mutate(se_Whitesearch = sigTest_Whitesearch,
         se_WhiteINner = sigTest_WhiteInner)
```
...
whiteGWR$coef_whiteInner <- replace(whiteGWR$coef_whiteInner, 
                    whiteGWR$se_WhiteINner < 0,
                    NA) 
...
tm4 <- tm_shape(whiteGWR) + 
  tm_fill("coef_whiteInner", 
          palette= "viridis",
          style = "jenks",
          title = "GWR, Inner London\ncoeff.(Jenks)",
          textNA = "Non-significant",
...
tt <- tmap_arrange(tm1, tm2, tm3, tm4)
tmap_save(tt, "output/GWR_map.png",dpi=500)
tt
```
...
model_a <- regression_data %>% 
  lm(asian_arrest_rate ~ asian_search_rate + inner_outer, data=.)
...
ggplot(all_data, aes(x=(no_qualification))) + 
  geom_histogram()
```
...
### Arrest ~ Crime + Inner/outer

```{r}
model_l2 <- lm(arrest_log ~ income_deprivation_rate,
                  data = regression_data2)
summary(model_l2) 
```

### Arrest ~ other factors 
