...
ggplot(all_data, aes(x=(unemp_rate))) + 
  geom_histogram()
```
...
```{r}
# build model 
model_mlr <- lm(search_log ~ crime_log  + median_income + unemp_log
                + income_deprivation_rate + income_old_deprivation_rate
                + bad_health_rate + cars_rate +inner_outer + lone_parent_rate
...
```{r}
# build model 
model_final <- lm(search_log ~ crime_log  
                  + obesity_aged16
...
```{r}
#calculate kernel bandwidth
GWRbandwidth <- gwr.sel(search_log ~ crime_log  
                  + obesity_aged16
                  + lone_parent_with_children_log
                  + inner_outer
                  + cars_rate,
...
regression_data2 <- regression_data2 %>%
  mutate(coef_crimeLog = results$crime_log,
         ...
         coef_inner = results$inner_outerInner.London,
...
sigTest_inner = abs(model_gwr$SD$inner_outerInner.London) - (2* model_gwr$SDF$inner_outerInner.London_se)
...
finalGWR$coef_inner <- replace(finalGWR$coef_inner, 
                    finalGWR$se_inner < 0,
                    NA) 
...
f2 <- tm_shape(finalGWR) + 
  tm_fill("coef_inner", 
          palette= "-RdYlBu",
          style = "jenks",
          title = "GWR, inner London coefficients",
          textNA = "Non-significant",
          title.fontface = "bold",
          alpha = 0.8)+  
  tm_borders(col = NA, alpha = 0.1) +
