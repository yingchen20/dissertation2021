...
1.  Is there a relationship between S&S (s60) search rate and arrest rate? -- Poisson regression
    ...
    -   What about for different minority groups, will the outcome be different? - Poisson regression
...
all_data <- all_data %>% 
  mutate(total_search_rate1000 = total_ss60/all_population *1000,
         total_arrest_rate1000 = total_arrest/all_population *1000) %>% 
  mutate_if(is.numeric, ~round(., 2))

# transformation
symbox(~total_search_rate1000, 
       all_data, 
       na.rm=T,
       powers=seq(-3,3,by=.5))

ggplot(all_data, aes(x=log(total_search_rate1000)))+ 
...
symbox(~total_arrest_rate1000, 
       all_data, 
       na.rm=T,
       powers=seq(-3,3,by=.5))

ggplot(all_data, aes(x=log(total_arrest_rate1000)))+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 0.5) + 
  geom_density(colour="red", 
               size=1, 
...
# scatter plot
qplot(x = log(total_search_rate1000), 
           y = log(total_arrest_rate1000),
           data = all_data) +
...
regression_data <- all_data %>% 
  dplyr::select(ward_code, total_search_rate1000, total_arrest_rate1000) %>% 
  mutate(total_search_rate1000log = log(total_search_rate1000),
         total_arrest_rate1000log = log(total_arrest_rate1000)) %>% 
  st_drop_geometry()

# convert Inf to 1e-9 and remove rows containing NA
...
model1 <- regression_data %>%
  lm(total_arrest_rate1000log ~ total_search_rate1000log, data=.)
...
# calculate the global moran's I for total stop and search 
...
```{r}
ggplot(all_data, aes(x=total_ss60))+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 5) + 
  geom_density(colour="red", 
               size=1, 
...
ggplot(all_data, aes(x=total_arrest))+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 1) + 
  geom_density(colour="red", 
               size=1, 
...
```{r}
mean(all_data$total_arrest)
var(all_data$total_arrest)
```
...
d <- deviance(model2)
par(mfrow=c(2,2))
plot(all_data$total_ss60, rsid)
boxplot(rsid~total_ss60,xlab="S&S",ylab = "Std residuals",data = all_data)
