...
# add population data at borough level
pop_boro <- read_csv("https://data.london.gov.uk/download/london-borough-profiles/c1693b82-68b1-44ee-beb2-3decf17dc1f8/london-borough-profiles.csv", na = c("NA", "n/a")) %>% 
  clean_names()

# trim unuseful rows 
pop_boro <- pop_boro[1:33,]

# merge all df together 
merged_boro <- left_join(ss_count_boro, arrest_count_boro) %>% 
  left_join(., crime_count_boro) %>% 
  left_join(., pop_boro %>% dplyr::select(code, gla_population_estimate_2017),
            by = c("GSS_CODE" = "code")) %>% 
...
Calculate the most dominant crime type in each borough and its percentage

```{r warning=F, message=F}
totalCrimeTab <- crime_london %>% 
  group_by(boro_name) %>% 
  summarise(., count=n(),) %>% 
  mutate(total_crime_prop = count/sum(count) *100) %>% 
  mutate_if(is.numeric, ~round(., 1)) %>% 
  st_drop_geometry()

# Select the row with the maximum value in each group
boroCrimeTab <- crime_london %>% 
  group_by(boro_name, crime_type) %>% 
  summarise(., sub_crime_count=n(),) %>% 
  group_by(boro_name) %>% 
  top_n(1, sub_crime_count) %>% 
  st_drop_geometry()

mergedCrimeTab <- left_join(totalCrimeTab, boroCrimeTab, by="boro_name") %>% 
  mutate(major_crime_prop = sub_crime_count/count *100) %>% 
  rename(total_crimes = "count",
         major_crimes = "sub_crime_count") %>% 
  mutate_if(is.numeric, ~round(., 1)) %>% 
  as.data.frame()

# adding the dominant crime type to the previous df
merged_boro_all <- left_join(merged_boro, mergedCrimeTab, by="boro_name")
```

```{r results='asis'}
mergedCrimeTab %>% arrange(desc(total_crime_prop)) %>% 
  kable()
```
...
a <- all_data_plot %>% 
  dplyr::select(black_search_rate, black_search_compare)

tm_shape(all_data_plot) + 
  tm_fill("black_search_compare", palette="viridis")+
  tm_legend(show=T)+
