...
2.  What are the factors that might lead to variation in search rate across the city?
    ...
    -   Is there a relationship between search rate and violent crime rate? How about factors such as lone parent unemployment rate, mean income rate, economic inactivity rate?

```{r}
colnames(all_data)
```

### total search vs. total arrest
...
library(ggpubr)
library(car)
# to make results easy to interpret, I divide search counts and arrest counts by population and then multiply by 1000, meaning the predicted number of arrest per 1,000 population in London wards, rather than per population.
...
Moran's I statistic is 0.04, showing no spatial autocorrelation in my residuals. Linear regression is suitable for my rate data is okay but what if using count data in a Poisson regression model...
...
```{r}
library(sandwich)
cov.mdl2 <- vcovHC(model2, type="HC0")
...
```{r}
c(deviance(model2), 1-pchisq(model2$deviance, model2$df.residual))  # GOF test
```
...
anova(model0, modelinb, test = "Chisq")
```

P-value =1 meaning the model does not predict values well, so should stick with Poisson regression.

```{r}
c(deviance(modelinb), 1-pchisq(modelinb$deviance, modelinb$df.residual)) 
```

Plot the predicted equation from Poisson regression and linear regression back to our scatter plot

log(y) = -2.89 + 0.008 ss60 (Poisson)

y = 0.0079 + 0.0424 ss60 (Linear)
...
ggplot(regression_data,
  ...
  stat_function(fun = Lfun, geom = "line", col="red") +
  geom_label(x=Inf, y = Inf, 
           label = "y = 0.0689 + 0.0395*x \n R-sqr = 0.715", vjust=1, hjust=1)
```

### White search vs white arrest
...
ggplot(regression_data,
  ...
  stat_function(fun = Lfun, geom = "line", col="red") +
  geom_label(x=Inf, y = Inf, 
           label = "y = 0.00594 + 0.0433*x \n R-sqr = 0.4917", vjust=1, hjust=1)
```

### Asian search vs. Asian arrest
...
ggplot(regression_data,
  ...
  stat_function(fun = Lfun, geom = "line", col="red") +
  geom_label(x=Inf, y = Inf, 
           label = "y = -0.00199 + 0.0344*x \n R-sqr = 0.322", vjust=1, hjust=1)
```

### total seach vs. multiple factors 
