...
ss60_count <- st_join(ss60_london, wardMap) %>% 
  ...
  mutate(total_ss60 = sum(c_across(Black:Other), na.rm = TRUE)) %>% 
...
```{r message=FALSE, warning=FALSE}
arrest_count <- st_join(ss60_london, msoaMap) %>% 
  filter(outcome=="Arrest") %>% 
  ...
  mutate(total_arrest = sum(c_across(White:Other), na.rm = TRUE)) %>% 
...
merged <- left_join(msoaDataMerged, vio_count) %>% 
  left_join(., ss60_count) %>% 
  left_join(., arrest_count)
...
```{r message=FALSE}
all_data <- subset(merged, select = -c(msoa_name)) %>% 
  clean_names() %>%   
  # violent crime rate 
  ...
  # hit rates
  mutate(black_arrest_rate = arrest_black/ ss60_black *100) %>% 
  mutate(white_arrest_rate = arrest_white/ ss60_white *100) %>% 
  mutate(asian_arrest_rate = arrest_asian/ ss60_asian *100) %>% 
  mutate(other_arrest_rate = arrest_other/ ss60_other *100) %>% 
  mutate(na_arrest_rate = arrest_na/ ss60_na *100) %>% 
  mutate_if(is.numeric, ~round(., 2))
...
aP <- sum(as.numeric(all_data$ethnic_group_2011_census_asian_asian_british), na.rm = TRUE)
bP <- sum(as.numeric(all_data$ethnic_group_2011_census_black_african_caribbean_black_british), na.rm = TRUE)
wP <- sum(as.numeric(all_data$ethnic_group_2011_census_white), na.rm = TRUE)
oP <- sum(as.numeric(all_data$ethnic_group_2011_census_other_ethnic_group), na.rm = TRUE)
