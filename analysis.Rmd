...
ss60_london %>% 
  ...
  kable(., caption = ' Proportion of S&S by race and population')
```
...
From the output graph, we can see that up until distances of around 450 meters, S&S(s60) appear to be clustered in London MSOAs and the largest bulge in the graph at around 400 meters Increasing the distance beyond 450 meters leads the distribution becomes random and dispersed. Therefore, setting eps to 400m or 450m is reasonable.
...
fviz_cluster(db, ss60_londonPoints, 
                           geom = "point",
...
It gives us 33 unique clusters. The graph tells us this high intensity (450 meters radius --\> 50 S&S for violence) is not widespread the country, only shown in few neighborhoods at Northen London.
...
fviz_cluster(db_KNN, ss60_londonPoints, 
                           geom = "point",
                           stand = FALSE, labelsize = NA,
                           outlier.pointsize = .8,
                           #xlab="x", ylab="y",
...
It gives us 5 unique clusters. This graph tells us if we increase the neighborhood radius range to 1.6 km <more explaination needed>
...
### Proportion of S&S 60 by race relative to their population

```{r message=False, warning=False, results='asis'}
a <- sum(as.numeric(all_data$ss60_asian), na.rm = TRUE) / 
  sum(as.numeric(all_data$ethnic_group_2011_census_asian_asian_british), na.rm = TRUE) *100
b <- sum(as.numeric(all_data$ss60_black), na.rm = TRUE) / 
  sum(as.numeric(all_data$ethnic_group_2011_census_black_african_caribbean_black_british), na.rm = TRUE) *100
w <- sum(as.numeric(all_data$ss60_white), na.rm = TRUE) / 
  sum(as.numeric(all_data$ethnic_group_2011_census_white), na.rm = TRUE) *100
o <- sum(as.numeric(all_data$ss60_other), na.rm = TRUE) / 
  sum(as.numeric(all_data$ethnic_group_2011_census_other_ethnic_group), na.rm = TRUE) *100
na <- sum(as.numeric(all_data$ss60_na), na.rm = TRUE) / 
  sum(as.numeric(all_data$age_structure_2011_census_all_ages), na.rm = TRUE) *100

data.frame(race = c("asian", "black", "white", "others", "NA"),
                         prop_by_pop = c(a, b, w, o, na)) %>% 
  mutate_if(is.numeric, ~round(., 2)) %>% 
  arrange(desc(prop_by_pop)) %>% 
  kable(.)
```

### Hit rates

That is, what proportion of searches, by race, were successful? This proportion is known as the contraband recovery rate, or the "hit rate." If racial groups have different hit rates, it can imply that racial groups are being subjected to different standards.
...
left_join(HR1, HR2) %>% 
  mutate(hit_rate = arrest/allSS *100) %>% 
  mutate_if(is.numeric, ~round(., 2)) %>% 
...
Make a table of all statical results for all my variables [**Q: How to turn the results into a dataframe with common column names (i.e. Moran I statistic, Expectation, Variance) and each variable's results fill in new rows?**]{.ul}
