...
ggplot(all_data, aes(x=log(total_arrest)))+ 
...
ggplot(all_data, aes(x=log(total_ss60)))+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 0.5) + 
  geom_density(colour="red", 
               size=1, 
...
regression_data <- all_data %>% 
  dplyr::select(ward_code, total_arrest, total_ss60) %>% 
  mutate(total_arrest_log = log(total_arrest),
         total_ss60_log = log(total_ss60)) %>% 
  st_drop_geometry()
...
model1 <- regression_data %>%
  lm(total_arrest_log ~ total_ss60_log, data=.)
...
```{r}
# plot the residuals
regression_data <- left_join(regression_data, 
...
# find a proper transformation for total search rate 
symbox(~total_search_rate, 
       all_data, 
       na.rm=T,
       powers=seq(-3,3,by=.5))

ggplot(all_data, aes(x=(total_search_rate)^-0.5))+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 0.5) + 
  geom_density(colour="red", 
               size=1, 
...
```{r message=FALSE, warning=FALSE}
# find a proper transformation for total hit rate 
symbox(~total_hit_rate, 
       all_data, 
       na.rm=T,
       powers=seq(-3,3,by=.5))

ggplot(all_data, aes(x=(total_hit_rate)^-0.5))+ 
  geom_histogram(aes(y = ..density..),
                 binwidth = 0.1) + 
  geom_density(colour="red", 
               size=1, 
...
```{r message=FALSE, warning=FALSE}
q <- qplot(x = (total_search_rate)^-0.5, 
           y = (total_hit_rate)^-0.5,
           data = all_data)
