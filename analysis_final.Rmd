...
> -   "indices_of_deprivation_idaci_2010",
> -   "indices_of_deprivation_idaopi_2010",
>
> *the above might be useful in spatial autocorrelation and regression model*
...
wardDataMerged <-
  ...
  rename(all_population = "population_and_age_all_ages_2011",
         ...
         ward_name = "NAME",
         ward_code = "GSS_CODE",
         idaci_index = "indices_of_deprivation_idaci_2010",
         idaopi_index = "indices_of_deprivation_idaopi_2010") 
...
```{r}
all_rates <- all_data %>% 
  dplyr::select(ends_with("rate") | ward_code | idaci_index | idaopi_index) 
```
...
moranI_idaci <- all_rates %>% 
  pull(idaci_index) %>%
  as.vector()%>%
  localmoran(., Lward_lw, zero.policy = TRUE)%>%
  as_tibble()

moranI_idaopi <- all_rates %>% 
  pull(idaopi_index) %>%
  as.vector()%>%
  localmoran(., Lward_lw, zero.policy = TRUE)%>%
  as_tibble()
...
all_rates <- all_rates %>% 
  ...
  mutate(idaci_Iz = as.numeric(moranI_idaci$Z.Ii)) %>% 
  mutate(idaopi_Iz = as.numeric(moranI_idaopi$Z.Ii))
```
...
t10 <- tm_shape(all_rates) + 
  tm_polygons("idaci_Iz",
        style="fixed",
        breaks=breaks1,
        palette= "viridis",
        midpoint=NA,
  ...
  tm_layout(frame=FALSE,
            main.title.position = "left",
            main.title.fontface = "bold",
            main.title="Local Moran's I, idaci index", 
            main.title.size=1,
            legend.title.size = 0.5,
            legend.text.size = 0.3,
            legend.outside = FALSE,
...
t11 <- tm_shape(all_rates) + 
  tm_polygons("idaopi_Iz",
        style="fixed",
        breaks=breaks1,
        palette= "viridis",
        midpoint=NA,
  ...
  tm_layout(frame=FALSE,
            main.title.position = "left",
            main.title.fontface = "bold",
            main.title="Local Moran's I, idaopi index", 
            main.title.size=1,
            legend.title.size = 0.5,
            legend.text.size = 0.3,
            legend.outside = FALSE,
...
t=tmap_arrange(t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11)
...
```{r}
tm_shape(all_rates) + 
  tm_fill("black_hit_rate", palette = "viridis")
```
